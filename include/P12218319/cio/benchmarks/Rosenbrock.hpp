...
*/
#ifndef P12218319_CIO_ROSENBROCK_HPP
#define P12218319_CIO_ROSENBROCK_HPP

#include "Problem.hpp"

namespace P12218319 { namespace cio {
	template<const uint32_t DIMENTIONS>
	class P12218319_EXPORT_API Rosenbrock : public Problem<DIMENTIONS> {
	public:
		double P12218319_CALL operator()(typename Problem<DIMENTIONS>::InputArray& x) const override throw() {
			...
			for(uint32_t i = 0; i < DIMENTIONS; ++i) {
				y += std::pow((1-x[i]),2) + 100*std::pow((x[i+1]-Math.pow(x[i],2)),2);
			}
			return y;
		}
	};
